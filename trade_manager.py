...
# ADDED: Smart Hours Guard — learns which hours of the day have high
# historical loss rates and blocks new entries during those hours.
...
# ADDED: Volatility Guard — real-time, per-symbol volatility spike
# detector. Stateless (see volatility_guard.py docstring), so unlike
# the other two guards there is no restore_state()/get_state() to wire
# up here - just instantiation. Fail-safe: if the file is missing the
# bot runs exactly as before.
try:
    from volatility_guard import VolatilityGuard as _VolatilityGuard
    _VOLATILITY_GUARD_AVAILABLE = True
except Exception:
    _VOLATILITY_GUARD_AVAILABLE = False
    _VolatilityGuard = None  # type: ignore[assignment,misc]
...
class TradeManager:
    ...
    def __init__(self, config: Dict, binance_client=None):
        ...
        # ADDED: Volatility Guard — stateless, so instantiation order
        # relative to _load_state() doesn't matter (nothing to restore).
        if _VOLATILITY_GUARD_AVAILABLE and _VolatilityGuard is not None:
            self.volatility_guard = _VolatilityGuard(config=config)
        else:
            self.volatility_guard = None
            if not _VOLATILITY_GUARD_AVAILABLE:
                logger.warning(
                    "⚠️ VolatilityGuard unavailable "
                    "(volatility_guard.py missing) — volatility-spike blocking disabled."
                )
    ...
    def _load_state(self):
        ...
        try:
            ...
            # has its own internal lock - nesting would deadlock).
    ...
    def is_daily_loss_limit_reached(self) -> bool:
        ...
        whether the guard is enabled - immediately before returning, so
        any future recurrence is immediately traceable via server logs.
        """
        self._check_daily_loss_reset()
    ...
    # ------------------------------------------------------------------
    # ADDED: Volatility Guard public interface
    # ------------------------------------------------------------------

    def is_volatility_blocked(self, ohlc, symbol: str) -> bool:
        """
        Returns True when `symbol`'s current volatility is abnormally
        elevated relative to its own recent history and a new entry
        should be skipped right now. Called from bot_core._execute_trade()
        alongside the other entry guards, before ANY exchange action.
        Fail-open: returns False on any error so a guard bug never
        stops trading entirely.
        """
        if self.volatility_guard is None:
            return False
        try:
            return self.volatility_guard.is_blocked(ohlc, symbol)
        except Exception as exc:
            logger.warning(f"VolatilityGuard.is_blocked error for {symbol}: {exc}")
            return False

    def get_volatility_status_text(self) -> str:
        """One-line status for Telegram /status. Always safe to call."""
        if self.volatility_guard is None:
            return "🔕 Not available (volatility_guard.py missing)"
        try:
            return self.volatility_guard.get_status_text()
        except Exception:
            return "⚠️ status unavailable"
    ...
    def calculate_volume_rank_leverage_cap(self, symbol_rank: Optional[int]) -> Optional[int]:
        """
        ADDED (user request): caps leverage based on the coin's RELATIVE
        24h volume rank within the scanned Top-N universe (rank 1 =
        highest volume that scan cycle) - complementary to
        calculate_dynamic_leverage() directly above, which caps based on
        PRICE volatility instead. A coin can show calm recent price
        action (the volatility check above wouldn't reduce leverage)
        while still sitting near the bottom of the Top-N list - meaning
        its order book is comparatively thin, so a STOP_MARKET order can
        suffer real slippage on a sudden move even though nothing in
        recent candles hinted at it.

        Uses RANK, not an absolute $ volume threshold, so this
        automatically adapts to overall market conditions - "the bottom
        third of today's Top-N" is always the relatively thinner group,
        in a quiet market or a frothy one alike.

        Returns None (no additional cap) whenever the rank is unknown
        (feature disabled, or this symbol's rank wasn't available this
        scan cycle) - fails OPEN exactly like every other guard in this
        bot when its input data isn't available, rather than guessing.
        The caller (bot_core._execute_trade) simply omits None from its
        min(...) leverage calculation, so "no cap" truly means no
        additional restriction - not a silent 0 or a crash.
        """
        if symbol_rank is None:
            return None
        if not self.config.get("VOLUME_LEVERAGE_CAP_ENABLED", True):
            return None

        tier1_max_rank = self.config.get("VOLUME_LEVERAGE_TIER1_MAX_RANK", 15)
        tier2_max_rank = self.config.get("VOLUME_LEVERAGE_TIER2_MAX_RANK", 30)

        if symbol_rank <= tier1_max_rank:
            return None  # most liquid tier - no additional cap beyond volatility's own
        elif symbol_rank <= tier2_max_rank:
            return self.config.get("VOLUME_LEVERAGE_TIER2_MAX_LEVERAGE", 5)
        else:
            return self.config.get("VOLUME_LEVERAGE_TIER3_MAX_LEVERAGE", 3)
