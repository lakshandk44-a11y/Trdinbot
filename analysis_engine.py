...
class AnalysisEngine:
    ...
    def calculate_all_indicators(self, ohlc: pd.DataFrame,
        ...
        # of this tally.
        #
        # ADDED (user request): Tool 5 previously had only 2 possible
        # sub-concepts total (trend, structure_broken) - the ONLY tool of
        # the 5 where MIN_SUBCONCEPTS_PER_TOOL=2 meant "100% of everything
        # this tool can ever say must agree", unlike Tool 1 (12 possible
        # sub-concepts, 2 required = ~17%) or even Tool 4 (3 possible, 2
        # required = ~67%). This made Tool 5 disproportionately hard to
        # ever count as "agreeing" the moment MIN_SUBCONCEPTS_PER_TOOL was
        # raised above 1, compared to every other tool.
        #
        # Two MORE real, independently-meaningful sub-concepts are added
        # below - both already fully computed by _market_structure() above
        # (bos, last_bos_direction, ema_bullish, ema_bearish are all
        # already on the result dict; _market_structure() itself is NOT
        # modified by this change, not even one line):
        #
        #   3. BOS specifically (continuation break) - distinct from CHoCH.
        #      structure_broken alone can't tell a continuation break (BOS)
        #      apart from a reversal break (CHoCH) - both set
        #      structure_broken to the same direction. bos + a matching
        #      last_bos_direction confirms this was specifically a BOS
        #      (bos is ONLY ever set True in the BOS branch of
        #      _market_structure() - never in the CHoCH branch - so this
        #      cannot double-fire on a CHoCH-only structure break).
        #   4. EMA alignment - re-introduced, but this time as one path
        #      among four instead of one among three. The original
        #      regression happened when EMA was the ONLY extra concept
        #      (3 total, 2 required = the same "must-agree" tie risk this
        #      change is fixing for Tool 5's base case). With bos also
        #      added as a 4th independent path, trend+structure_broken
        #      alone still reaches min_sub=2 exactly as before whenever an
        #      actual break has happened (nothing that used to count stops
        #      counting) - EMA now only ever ADDS an extra way to reach
        #      min_sub=2 on candles where no break has happened yet, it
        #      never subtracts one.
        ms = results["market_structure"]
        ms_bos_bullish = bool(ms.get("bos")) and ms.get("last_bos_direction") == "up"
        ms_bos_bearish = bool(ms.get("bos")) and ms.get("last_bos_direction") == "down"
        ms_bull = sum([
    ...
    def _calculate_profit_chance(self, results: Dict) -> float:
        ...
        # Open Interest Confluence (ADDED, user request): crypto-futures-
        # specific positioning signal, similar spirit to Funding Rate
        # directly above but reading the OPPOSITE side of the picture -
        # not "how expensive is it to stay in this trade" (funding) but
        # "is real new money actually entering, or is this move just
        # existing positions unwinding".
        #
        # IMPORTANT - unlike Funding Rate, this is intentionally NOT
        # direction-dependent. The standard professional interpretation
        # is a symmetric 4-quadrant read:
        #   price up   + OI up   -> genuinely new longs   -> STRONG move
        #   price up   + OI down -> short covering         -> WEAK move
        #   price down + OI up   -> genuinely new shorts   -> STRONG move
        #   price down + OI down -> long liquidation        -> WEAK move
        # In BOTH the bullish and bearish case, rising OI means the move
        # has fresh conviction behind it, and falling OI means it's
        # being driven by existing positions closing - so rising OI
        # always rewards the setup and falling OI always penalizes it,
        # regardless of which direction the 5-tool vote is currently
        # leaning. (An earlier draft of this logic incorrectly mirrored
        # Funding Rate's direction-flip structure - caught and corrected
        # via testing before this ever reached a live symbol.)
        #
        # Whenever oi_change_pct is absent (fetch failed, disabled, or
        # symbol too new for enough OI history), this contributes
        # exactly 0 - identical no-op behavior to every other confluence
        # component here when its data is unavailable.
        oi_change_pct = md.get("oi_change_pct")
        if oi_change_pct is not None:
            oi_threshold = self.config.get("OI_CHANGE_THRESHOLD_PCT", 1.5)
            if oi_change_pct > oi_threshold:
                score += 3   # OI rising: genuinely new positioning entering - confirms real conviction
            elif oi_change_pct < -oi_threshold:
                score -= 3   # OI falling: existing positions closing - move looks like unwinding
        ...
        _oi_pts = 0
        if oi_change_pct is not None:
            _oi_threshold = self.config.get("OI_CHANGE_THRESHOLD_PCT", 1.5)
            if oi_change_pct > _oi_threshold:
                _oi_pts = 3
            elif oi_change_pct < -_oi_threshold:
                _oi_pts = -3
        ...
        logger.debug(f"Score breakdown: raw={raw_score:.1f} | "
                     ...
                     f"oi={_oi_pts:+d} | regime={_regime_pts:+d} (regime={regime})")
