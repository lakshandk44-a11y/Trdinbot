...
class HackerAIBot:
    ...
    def _filter_disabled_coins(self, coins: List[str]) -> List[str]:
        ...
        Removes any symbol the user has manually switched OFF via the new
        Telegram "🪙 Coin List View" menu from this cycle's scan list -
        so a disabled coin gets no analysis and can never open a brand
        new trade, exactly like the existing guards (Trading Hours,
        Volatility Guard, etc.) block new entries without touching
        anything already open.

        CRITICAL SAFETY RULE: a symbol that already has an OPEN trade is
        NEVER dropped here, even if it's currently disabled. bot_core.
        _scan_coins_247() only calls trade_manager.update_trade_price()
        for symbols present in the list it's given, and trade_manager.
        _evaluate_trade() (SL/TP/trailing) only acts on that same
        `current_price` - so removing an in-trade symbol from this list
        would silently stop its price from ever updating again, freezing
        its SL/TP/trailing forever. Disabling a coin therefore only ever
        blocks a NEW position on that symbol; a trade already open when
        you disable it keeps being scanned/managed completely normally
        until it closes on its own SL/TP, exactly as if it were still
        enabled.

        Fails OPEN (returns `coins` unchanged) whenever the Telegram
        control panel isn't available/enabled or nothing is disabled -
        never blocks scanning on uncertainty, same convention as every
        other guard in this file.
        """
        controller = getattr(self, "telegram_controller", None)
        if not controller or not getattr(controller, "enabled", False):
            return coins
        disabled = getattr(controller, "disabled_coins", None)
        if not disabled:
            return coins
        open_trades = self.trade_manager.open_trades
        return [c for c in coins if c not in disabled or c in open_trades]
