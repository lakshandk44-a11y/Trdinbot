...
# Optional Coin Performance Auto-Guard (user request) - same fail-safe
# pattern: if unavailable or COIN_PERFORMANCE_GUARD_ENABLED=False, the
# bot's normal scan/decision/execute path is 100% unaffected.
try:
    from coin_performance_guard import CoinPerformanceGuard
    _COIN_PERF_GUARD_AVAILABLE = True
except Exception:
    _COIN_PERF_GUARD_AVAILABLE = False
...
class BinanceFuturesClient:
    ...
    def __init__(self, api_key: str, api_secret: str, testnet: bool = False, timeout: int = 10):
        ...
        # BUG FIX (user-reported): stored once here and applied to every
        # HTTP call this client makes (see each method below) - previously
        # NO call had any timeout, so a slow/unresponsive Binance endpoint
        # could hang the bot indefinitely. See the config.py comment next
        # to API_REQUEST_TIMEOUT_SECONDS for the full explanation.
    ...
    def ping(self) -> dict:
        resp = self.session.get(f"{self.base_url}/fapi/v1/ping", timeout=self.timeout)
        return resp.json()
    ...
    def exchange_info(self) -> dict:
        resp = self.session.get(f"{self.base_url}/fapi/v1/exchangeInfo", timeout=self.timeout)
        return resp.json()
    ...
    def klines(self, symbol: str, interval: str, limit: int = 100,
        ...
        resp = self.session.get(f"{self.base_url}/fapi/v1/klines", params=params, timeout=self.timeout)
        return resp.json()

    def ticker_24hr(self) -> list:
        resp = self.session.get(f"{self.base_url}/fapi/v1/ticker/24hr", timeout=self.timeout)
        return resp.json()
    ...
    def ticker_price(self, symbol: str = None) -> dict:
        ...
        if symbol:
            params["symbol"] = symbol
        resp = self.session.get(f"{self.base_url}/fapi/v1/ticker/price", params=params, timeout=self.timeout)
        data = resp.json()
        if symbol:
            return data
        return data
    ...
    def funding_rate(self, symbol: str) -> dict:
        ...
        """
        resp = self.session.get(f"{self.base_url}/fapi/v1/premiumIndex", params={"symbol": symbol}, timeout=self.timeout)
        return resp.json()

    def open_interest_hist(self, symbol: str, period: str = "15m", limit: int = 30) -> list:
        """
        ADDED (user request): GET /futures/data/openInterestHist - public
        endpoint (same unsigned pattern as klines/funding_rate above, no
        signing needed). Returns historical open-interest snapshots for a
        symbol at the given period granularity ("5m","15m","30m","1h",
        "2h","4h","6h","12h","1d"), most recent `limit` readings, oldest
        first. Each entry has sumOpenInterest (total open contracts
        across the whole market for this symbol right now) and a
        timestamp (ms). Only the last ~30 days of history is available -
        a limitation of this specific Binance endpoint itself, not
        something this bot controls. Note this endpoint lives under
        /futures/data/ rather than /fapi/v1/ like most other endpoints on
        this client - that's Binance's own API layout, not a typo.
        """
        params = {"symbol": symbol, "period": period, "limit": limit}
        resp = self.session.get(f"{self.base_url}/futures/data/openInterestHist", params=params, timeout=self.timeout)
        return resp.json()
...
class HackerAIBot:
    ...
    def __init__(self, config: Dict):
        ...
        self.client = BinanceFuturesClient(
            ...
            timeout=config.get("API_REQUEST_TIMEOUT_SECONDS", 10)
        )
        ...
        self.analysis_cache = {}
        self.scan_count = 0
        # ADDED: symbol -> 24h-volume rank (1 = highest volume), rebuilt
        # fresh every scan cycle inside _get_top_coins() below. Read by
        # _execute_trade() for the volume-rank leverage cap. Starts empty
        # so any lookup before the first successful scan correctly
        # returns None (unknown rank) - trade_manager.calculate_volume_
        # rank_leverage_cap() then fails OPEN, exactly as intended.
        self.symbol_volume_rank = {}
        # ADDED (user request - Telegram Coin List View / manual per-coin
        # ON/OFF switches): cache of the exact coin universe from the most
        # recent _get_top_coins() call, so the Telegram coin menu can show
        # the SAME 50 coins the bot is actually scanning this cycle without
        # making its own separate Binance call. Purely a read-only cache of
        # data already being fetched - starts empty and falls back to the
        # static TOP_N_COINS list (see telegram_control._get_scan_universe)
        # until the first scan cycle populates it.
        self.last_scanned_coins = []
        # ADDED (user request - No-Trades Watchdog, future-proofing for
        # TRADING_HOURS_FILTER being re-enabled): timestamp of the last
        # time ANY new trade was successfully opened, anywhere. Starts at
        # bot startup (not None) so a freshly-started bot doesn't
        # immediately think it's been "quiet too long" before it's even
        # had a real chance to trade. Updated in _execute_trade() the
        # moment a new trade actually opens; read by
        # _check_no_trades_watchdog() every scan cycle.
        self.last_trade_opened_at = datetime.now()
        self.last_no_trades_alert_sent_at = None
        ...
        # Coin Performance Auto-Guard (user request) - tracks each scanned
        # coin's own win/loss record (trade_manager.trade_history) and can
        # auto-disable a chronically losing coin, permanently or for a
        # 30-day cooldown - see coin_performance_guard.py. Never blocks/
        # crashes the bot if unavailable; fully opt-in via
        # COIN_PERFORMANCE_GUARD_ENABLED (Telegram-toggleable, same menu
        # grid as every other guard).
        self.coin_performance_guard = None
        if _COIN_PERF_GUARD_AVAILABLE:
            try:
                self.coin_performance_guard = CoinPerformanceGuard(self, config)
            except Exception as e:
                logger.warning(f"⚠️ Coin Performance Guard could not be initialized: {e}")
    ...
    def _main_loop(self):
        ...
        while self.running:
            try:
                ...
                # ADDED (user request - Auto-Performance Tracking): re-check
                # every currently-scanned coin's own win/loss record and
                # apply any new auto-disable/cooldown-expiry decision BEFORE
                # this cycle's filtering below, so a coin that just crossed
                # a threshold is excluded starting THIS scan, not next one.
                # Purely local/in-memory - zero extra API calls. See
                # _run_coin_performance_guard() for details.
                self._run_coin_performance_guard()

                # ADDED (user request - Telegram Coin List View / manual
                # per-coin ON/OFF switches): drop any coin manually turned
                # OFF from Telegram before this scan cycle, so a disabled
                # coin gets zero analysis calls and can never open a new
                # trade. See _filter_disabled_coins() for why a coin that
                # already has an OPEN trade is never dropped here.
                coins_to_scan = self._filter_disabled_coins(coins_to_scan)
                ...
                # ADDED (user request - No-Trades Watchdog,
                # future-proofing): once per cycle, check whether the bot
                # has gone quiet (zero new trades) for too long while
                # actively running - see _check_no_trades_watchdog().
                self._check_no_trades_watchdog()
    ...
    def _get_top_coins(self) -> List[str]:
        ...
        try:
            ...
            if top_coins:
                coins = top_coins
                # ADDED: rebuild the rank map from the SAME data just
                # fetched above - zero extra API calls, 100% real,
                # current-cycle data. rank 1 = highest 24h quoteVolume.
                # On any exception below (network/parsing), this whole
                # try block is skipped and self.symbol_volume_rank simply
                # keeps its previous cycle's values (still recent, at
                # most one scan interval stale) rather than being wiped -
                # calculate_volume_rank_leverage_cap() only ever reads
                # this, never assumes it's perfectly fresh.
                self.symbol_volume_rank = {
                    sym: rank for rank, sym in enumerate(top_coins, start=1)
                }
        except Exception:
            pass
        # ADDED (user request - Telegram Coin List View): cache the exact
        # coin universe just computed above (live top-N, or the static
        # fallback list on fetch failure) - see self.last_scanned_coins
        # above for why. This is only a cache write; it does not change
        # what `coins` this function returns or anything about scanning.
        self.last_scanned_coins = coins
        return coins

    def _filter_disabled_coins(self, coins: List[str]) -> List[str]:
        """
        ADDED (user request - Telegram Coin List View / manual per-coin
        ON/OFF switches; extended for Auto-Performance Tracking).

        Removes any symbol that's currently OFF for either of TWO
        independent reasons from this cycle's scan list, so a disabled
        coin gets no analysis and can never open a brand new trade,
        exactly like the existing guards (Trading Hours, Volatility
        Guard, etc.) block new entries without touching anything already
        open:
          1. MANUAL - switched OFF via the Telegram "🪙 Coin List View"
             menu (telegram_controller.disabled_coins).
          2. AUTO - auto-disabled by the Coin Performance Guard due to a
             poor recent win rate (coin_performance_guard.disabled_
             symbols()), permanently or for a 30-day cooldown.
        A coin is skipped if EITHER source says it's off (union) - either
        one is sufficient on its own, matching how each is presented/
        toggled independently in the Coin List View.

        CRITICAL SAFETY RULE (unchanged): a symbol that already has an
        OPEN trade is NEVER dropped here, even if it's currently
        disabled by either mechanism. bot_core._scan_coins_247() only
        calls trade_manager.update_trade_price() for symbols present in
        the list it's given, and trade_manager._evaluate_trade() (SL/TP/
        trailing) only acts on that same `current_price` - so removing
        an in-trade symbol from this list would silently stop its price
        from ever updating again, freezing its SL/TP/trailing forever.
        Disabling a coin (manually OR automatically) therefore only ever
        blocks a NEW position on that symbol; a trade already open when
        it gets disabled keeps being scanned/managed completely normally
        until it closes on its own SL/TP, exactly as if it were still
        enabled.

        Fails OPEN (returns `coins` unchanged) whenever neither source is
        available/enabled or nothing at all is disabled - never blocks
        scanning on uncertainty, same convention as every other guard in
        this file.
        """
        manual_disabled = None
        controller = getattr(self, "telegram_controller", None)
        if controller and getattr(controller, "enabled", False):
            manual_disabled = getattr(controller, "disabled_coins", None)

        auto_disabled = None
        guard = getattr(self, "coin_performance_guard", None)
        if guard and self.config.get("COIN_PERFORMANCE_GUARD_ENABLED", True):
            try:
                auto_disabled = guard.disabled_symbols()
            except Exception as e:
                logger.warning(f"Coin Performance Guard: disabled_symbols() failed: {e}")
                auto_disabled = None

        if not manual_disabled and not auto_disabled:
            return coins

        combined = set(manual_disabled or []) | set(auto_disabled or [])
        if not combined:
            return coins

        open_trades = self.trade_manager.open_trades
        return [c for c in coins if c not in combined or c in open_trades]

    def _run_coin_performance_guard(self):
        """
        ADDED (user request - Auto-Performance Tracking): once per scan
        cycle, asks the Coin Performance Guard to re-check every
        currently-scanned coin's own win/loss record and apply any new
        permanent-disable / cooldown-disable / cooldown-expiry decision
        (see coin_performance_guard.py for the exact rule), then sends a
        Telegram alert for each actual change. Purely local/in-memory
        (reads trade_manager.trade_history already held in memory - no
        extra Binance or Telegram polling calls), so safe to run every
        cycle. No-ops instantly if the guard isn't available - never
        blocks/affects scanning either way.
        """
        guard = getattr(self, "coin_performance_guard", None)
        if not guard:
            return
        try:
            guard.evaluate_and_apply()
            for alert in guard.pop_pending_alerts():
                self._send_coin_performance_alert(alert)
        except Exception as e:
            logger.warning(f"Coin Performance Guard: scan-cycle check failed: {e}")

    def _send_coin_performance_alert(self, alert: Dict):
        """
        ADDED (user request - Auto-Performance Tracking): formats and
        sends ONE Telegram alert for an auto-disable/auto-re-enable
        event, clearly tagged "🤖 Auto" (per explicit request, so it's
        never confused with a manual toggle from the Coin List View,
        which sends its own "👤 Manual"-tagged alert directly from
        telegram_control._handle_toggle_coin). Fire-and-forget, same
        fail-safe pattern as every other Telegram notification in this
        file - any failure here is caught/logged and never affects
        trading.
        """
        ...
        try:
            ...
            tier_line = ""
            if tier == "permanent":
                tier_line = "\nTier: 🚫 Permanent (needs a manual re-enable from the Coin List View)"
            elif tier == "cooldown":
                cooldown_days = self.config.get("COIN_PERF_COOLDOWN_DAYS", 30)
                tier_line = f"\nTier: ⏳ {cooldown_days}-day Cooldown (auto re-enables on its own)"

            message = (
                f"🪙 *Coin {action_label}*\n"
                f"Coin: {symbol}\n"
                f"Triggered by: 🤖 Auto (Performance Guard)"
                f"{tier_line}\n"
                f"{reason}"
            )
            send_telegram(message)
        except Exception as e:
            logger.warning(f"Coin Performance Guard: alert send failed: {e}")

    def _check_no_trades_watchdog(self):
        """
        ADDED (user request - future-proofing, specifically for
        TRADING_HOURS_FILTER being re-enabled).

        TRADING_HOURS_FILTER_ENABLED + a narrow ALLOWED_TRADING_HOURS_UTC
        window is DOCUMENTED (see config.py) to have once combined with a
        too-strict MIN_PROFIT_CHANCE and produced ZERO new trades for an
        extended period with nobody aware, since a bot that just keeps
        scanning normally gives no obvious signal that its filters have
        become too restrictive together. MIN_PROFIT_CHANCE has been
        corrected specifically to avoid repeating that exact combination
        (see its own config.py comment) - but rather than just trust that
        and move on, this actively watches for the SAME symptom
        happening again for ANY reason (a future config change, a market
        regime shift, anything) and says so via Telegram instead of
        staying silent.

        Fires at most ONE alert per NO_TRADES_ALERT_HOURS-or-more gap
        (never repeats sooner than NO_TRADES_ALERT_REPEAT_HOURS after
        that), and only while the bot is actually trying to trade -
        skipped entirely while paused or waiting out a balance issue,
        since zero trades is the CORRECT/expected state in either of
        those, not a problem to flag. Fails open/silent on any error or
        if disabled - never affects scanning/trading itself either way.
        """
        if not self.config.get("NO_TRADES_ALERT_ENABLED", True):
            return
        if self.paused or self.waiting_for_balance:
            return
        try:
            alert_after = timedelta(hours=self.config.get("NO_TRADES_ALERT_HOURS", 6))
            repeat_after = timedelta(hours=self.config.get("NO_TRADES_ALERT_REPEAT_HOURS", 6))
            now = datetime.now()

            quiet_for = now - self.last_trade_opened_at
            if quiet_for < alert_after:
                return
            if self.last_no_trades_alert_sent_at and (now - self.last_no_trades_alert_sent_at) < repeat_after:
                return

            self.last_no_trades_alert_sent_at = now
            if not _TELEGRAM_AVAILABLE:
                return
            hours_quiet = quiet_for.total_seconds() / 3600
            send_telegram(
                f"⚠️ *No New Trades*\n"
                f"No new trade has opened in {hours_quiet:.1f}h while the bot "
                f"was actively scanning.\n"
                f"Current filters (MIN_PROFIT_CHANCE, TRADING_HOURS_FILTER, and "
                f"the other guards) may be too strict together right now - "
                f"worth checking /status and the config."
            )
        except Exception as e:
            logger.warning(f"No-Trades Watchdog check failed: {e}")
    ...
    def _fetch_multi_timeframe(self, symbol: str) -> Optional[Dict[str, pd.DataFrame]]:
        ...
        # ---- Open Interest Confluence (ADDED, user request) -------------
        # Same additive, fully isolated pattern as Funding Rate directly
        # above: on any failure, this key is simply absent and the OI-
        # confluence check inside AnalysisEngine._calculate_profit_chance
        # no-ops - nothing else about this scan is affected. Fetches
        # recent open-interest history (same period as the execution/
        # "lower" timeframe by default) and reduces it to one number here
        # - the simple % change in total open interest across the
        # lookback window. The actual bullish/bearish INTERPRETATION
        # (does this confirm or contradict the current tool-vote
        # direction) deliberately happens inside AnalysisEngine, matching
        # exactly where Funding Rate's own interpretation already lives -
        # this method only ever fetches and reduces raw exchange data,
        # never makes a trading judgement itself.
        if self.config.get("OI_CONFLUENCE_ENABLED", True):
            try:
                oi_period = self.config.get("OI_HISTORY_PERIOD", "15m")
                oi_limit = self.config.get("OI_HISTORY_LOOKBACK", 20)
                oi_hist = self.client.open_interest_hist(symbol, period=oi_period, limit=oi_limit)
                if oi_hist and len(oi_hist) >= 2:
                    ...
                    if oi_first > 0:
                        ...
                        if "market_data" not in result:
                            result["market_data"] = {}
                        result["market_data"]["oi_change_pct"] = oi_change_pct
            except Exception as e:
                logger.debug(f"Open-interest fetch skipped for {symbol}: {e}")
    ...
    def _execute_trade(self, symbol: str, decision: str, signal: Dict, ohlc_data: Dict,
        ...
        # ADDED: Volatility Guard — skip entry if THIS symbol's current
        # volatility is abnormally elevated relative to its own recent
        # history. Uses the SAME ohlc_data already fetched for analysis
        # (no extra API call). Checked here before ANY exchange action,
        # same pattern as the other three entry guards above. Open
        # trades (on any symbol) are never affected — new entries only.
        volatility_tf = self.config.get("VOLATILITY_GUARD_TIMEFRAME", "lower")
        volatility_ohlc = ohlc_data.get(volatility_tf) if ohlc_data else None
        if volatility_ohlc is None and ohlc_data:
            volatility_ohlc = ohlc_data.get("lower")  # defensive fallback
        if self.trade_manager.is_volatility_blocked(volatility_ohlc, symbol):
            logger.info(
                f"📊 Volatility Guard: skipping {symbol} — "
                f"current volatility abnormally elevated vs recent history."
            )
            return
        ...
        # ADDED: Volume-Rank Leverage Cap - complementary, independent
        # risk factor to the price-volatility check directly above (see
        # config.py's VOLUME-RANK LEVERAGE CAP section for the full
        # rationale). Uses self.symbol_volume_rank, rebuilt fresh every
        # scan cycle inside _get_top_coins() from data already fetched
        # there - no extra API call here. Never blocks the trade and
        # never touches margin sizing - only ever tightens the LEVERAGE
        # multiplier for thinner-liquidity coins. .get(symbol) correctly
        # returns None (unknown rank) if this symbol somehow isn't in
        # the map, and calculate_volume_rank_leverage_cap() itself
        # returns None for "no additional cap" in that case - both fail
        # OPEN, matching every other guard in this bot.
        symbol_rank = self.symbol_volume_rank.get(symbol)
        volume_rank_cap = self.trade_manager.calculate_volume_rank_leverage_cap(symbol_rank)

        leverage_candidates = [trade_leverage, vol_leverage]
        if volume_rank_cap is not None:
            leverage_candidates.append(volume_rank_cap)
        final_leverage = min(leverage_candidates)
        ...
        try:
            ...
            if trade:
                ...
                # ADDED (user request - No-Trades Watchdog): mark the
                # moment of this successful new trade, so
                # _check_no_trades_watchdog() correctly resets its "quiet
                # too long" clock right when it should.
                self.last_trade_opened_at = datetime.now()
